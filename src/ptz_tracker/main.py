    ...
    while True:
        ...
        if app_state.tracking_state == State.DETECTION:
            ...
            if success and bbox:
                ...
                # Draw tracking visualization on PTZ frame
                ...
                # For debug mosaic - compute auxiliary stages on PTZ frame
                if app_state.show_debug:
                    ...
                    contours_frame = draw_contours_overlay(
                        ptz_frame.copy(), vc, list(vi)
                    )

                    # Optional detection overlay for the PTZ frame
                    ...
                    # Tracking stage mirrors the PTZ tracking visualization
            ...
            if new_bbox:
                ...
                # Continue searching on PTZ frame and populate stages (only if visible)
                if app_state.show_debug:
                    fg_mask = tracker.update_background_subtraction(ptz_frame)
                    cleaned_mask = tracker.clean_mask(fg_mask)
                    vc, vi = tracker.detect_contours(cleaned_mask)
